...
export const registerToken = async (patientId: string, doctorId: string, patientName: string, type: 'REGULAR' | 'EMERGENCY' = 'REGULAR'): Promise<{ success: boolean; token: Token; waitMinutes: number }> => {
    const response = await fetch(`${API_URL}/register`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({ patientId, doctorId, patientName, type })
    });
